#!/bin/sh
# sitemon - monitor dostupnosti saitov dlya Keenetic/Entware
...
echo $$ > "$PIDFILE"
load_conf
log "zapushen: name=$ROUTER_NAME sites=[$SITES] server=$SERVER_URL"
...
while :; do
    ...
    while [ "$s" -le "$SEND_ATTEMPTS" ]; do
        ...
        log "otpravka na server ne udalas (popytka $s)"
        s=$((s + 1))
    ...
    if [ "$sent" = 1 ]; then
        ...
        log "server nedostupen, cikl nachnetsya zanovo cherez $RETRY_DELAY s"
        nap "$RETRY_DELAY"
    fi
done
